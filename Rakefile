...
task :pylint do
  title("PyLint")
  puts `pylint --reports=n --disable-msg=#{pylint_disable} comicnamer/*.py tests/*.py`
end
...
task :topypi => :test do
  cur_file = File.open("comicnamer/__init__.py").read()
  comicnamer_version = cur_file.scan(/__version__ = \((.*)\)/)
  comicnamer_version = comicnamer_version[0][0].to_f

  puts "Build sdist and send comicnamer v#{comicnamer_version} to PyPi?"
  if $stdin.gets.chomp == "y"
    puts "Sending source-dist (sdist) to PyPi"

    if system("python setup.py sdist register upload")
      print "comicnamer uploaded!"
    end
